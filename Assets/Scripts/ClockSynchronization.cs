...
public class ClockSynchronization : MonoBehaviour
{
    ...
    public IEnumerator RunSyncInterval()
    {
        ...
        while(!Config_CoinTask.isSyncing)
        {
            yield return 0;
        }
        while (Config_CoinTask.isSyncing)
        {
            ...
            yield return new WaitForSeconds(Config_CoinTask.syncNTPInterval);
            ...
            Debug.Log("about to ask gameclock");
            long ntpTime = GameClock.GetClockMilliseconds(currentNTPTime);
            //Experiment_CoinTask.Instance.debugText.text = ntpTime.ToString();
            ...
            Debug.Log("composing message"); 
            string message = ntpTime.ToString() + "\t" + localTime.ToString();
            //Debug.Log("sending message to EPAD");
            //yield return StartCoroutine(syncboxControl.SyncImmediately());
            Debug.Log("about to send to epad");
