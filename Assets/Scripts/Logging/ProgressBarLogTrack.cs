...
public class ProgressBarLogTrack: LogTrack {
	...
	void LateUpdate () {
		...
		if(ExperimentSettings_CoinTask.isLogging && ( currentValue != myProgressBar.Value ) ){ //if the text has changed, log it!
			LogProgressBar ();
		}
		if(ExperimentSettings_CoinTask.isLogging && ( currentColor != currentImage.color ) ){ //if the text has changed, log it!
			LogColor ();
		}
	}
