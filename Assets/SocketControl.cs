        ...
        try
        {
            ...
            while (true)
            {
                ...
                while ((i = stream.Read(bytes, 0, bytes.Length)) != 0)
                {
                    ...
                    if (parseResult)
                    {
                        Config_CoinTask.neuralDeviceConnected = res;
                    }
                }
...
public class ClientThread : ThreadedJob
{
    ...
    protected override void ThreadFunction()
    {
        ...
        if (Config_CoinTask.directIPAddress.Contains("0.0.0.0"))
        {
        ...
        else
        {
            ...
            PrintSomething(Config_CoinTask.directIPAddress);
            serverAddress = IPAddress.Parse(Config_CoinTask.directIPAddress); // we are not doing TryParse here as we already did that in NetworkManager before switching to main experiment scene
        }
    ...
    IPAddress ClientSocket()
    {
        ...
        string message = "SUBJ \t" + Config_CoinTask.subjectName;
